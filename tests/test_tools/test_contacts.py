...
async def test_list(mock_backend):
    result = json.loads(await handle_contacts(mock_backend, ContactsOptions(action="list")))
...
async def test_search(mock_backend):
    result = json.loads(
        await handle_contacts(mock_backend, ContactsOptions(action="search", query="John")))
    assert result["contacts"] == []
    assert result["count"] == 0
...
async def test_search_missing_params(mock_backend):
    result = json.loads(await handle_contacts(mock_backend, ContactsOptions(action="search")))
...
async def test_add(mock_backend):
    result = json.loads(
        await handle_contacts(mock_backend, ContactsOptions(action="add", phone="+1234567890",
                first_name="John",
                last_name="Doe",
            ),))
...
async def test_add_missing_params(mock_backend):
    ...
    result = json.loads(
        await handle_contacts(mock_backend, ContactsOptions(action="add", first_name="John")))
    assert "error" in result
...
async def test_block(mock_backend):
    result = json.loads(
        await handle_contacts(mock_backend, ContactsOptions(action="block", user_id=123)))
...
async def test_unblock(mock_backend):
    result = json.loads(
        await handle_contacts(mock_backend, ContactsOptions(action="block", user_id=123, unblock=True))
    )
...
async def test_block_missing_params(mock_backend):
    result = json.loads(await handle_contacts(mock_backend, ContactsOptions(action="block")))
    assert "error" in result


@pytest.mark.asyncio
async def test_unknown_action(mock_backend):
    result = json.loads(await handle_contacts(mock_backend, ContactsOptions(action="unknown")))
    assert "error" in result
...
async def test_general_exception(mock_backend):
    ...
    result = json.loads(
        await handle_contacts(mock_backend, ContactsOptions(action="add", phone="+1", first_name="X"))
    )
    assert "error" in result
