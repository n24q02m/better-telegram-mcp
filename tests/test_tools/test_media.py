...
async def test_send_photo(mock_backend):
    result = json.loads(
        await handle_media(
            mock_backend, MediaOptions(action="send_photo", chat_id=123,
...
async def test_send_file(mock_backend):
    result = json.loads(
        await handle_media(
            mock_backend, MediaOptions(action="send_file", chat_id=123,
...
async def test_send_photo_missing_params(mock_backend):
    ...
    result = json.loads(
        await handle_media(
            mock_backend, MediaOptions(action="send_photo", file_path_or_url="https://example.com/photo.jpg",
            ),
        )
    )
    assert "error" in result
...
async def test_download(mock_backend):
    result = json.loads(
        await handle_media(
            mock_backend, MediaOptions(action="download", chat_id=123,
                message_id=10,
                output_dir="/tmp",
            ),
        )
