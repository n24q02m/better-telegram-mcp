...
def _is_under_blocked_prefix(
    ...
    """Return the blocked prefix that ``path`` falls under, else None.

    Robust against the macOS firmlink layout (``/etc`` -> ``/private/etc``):
    each blocked prefix directory is itself canonicalized with ``resolve()`` so
    the containment check compares like-for-like canonical paths. Containment uses
    ``Path.is_relative_to`` (a proper path-segment check) instead of a naive
    ``startswith``. Both the candidate and blocked prefixes are made absolute
    before comparison to ensure consistency.
    """
    try:
    ...
    for prefix in prefixes:
        ...
        blocked_dirs = {prefix_path}
        try:
            blocked_dirs.add(prefix_path.resolve())
        except OSError:
            pass

        for blocked in blocked_dirs:
            try:
                blocked_abs = blocked.absolute()
            except OSError:
                blocked_abs = blocked
            if candidate == blocked_abs or candidate.is_relative_to(blocked_abs):
                return prefix
    return None
...
def validate_file_path(file_path: str, *, allowed_dir: Path | None = None) -> Path:
    ...
    lexical = Path(os.path.normpath(Path(file_path).expanduser().absolute()))
    blocked = None
    for candidate in {path, lexical}:
        blocked = _is_under_blocked_prefix(candidate, _blocked_prefixes)
        if blocked:
            break
...
def validate_output_dir(output_dir: str, *, base_dir: Path | None = None) -> Path:
    ...
    lexical = Path(os.path.normpath(Path(output_dir).expanduser().absolute()))
    blocked = None
    for candidate in {path, lexical}:
        blocked = _is_under_blocked_prefix(candidate, _blocked_prefixes)
        if blocked:
