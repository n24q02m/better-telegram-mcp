...
async def _ask_for_input(
    ...
    try:
        ...
        await send_message(
            ...
            {
                "type": "error",
                "text": timeout_error_text or f"Timed out waiting for {field}.",
            },
        )
        return None
...
async def _relay_telethon_auth(
    ...
    # Step 1 & 2: Send OTP and ask for it
    ...
    otp_code = await _ask_for_input(
        ...
        field="otp_code",
        placeholder="12345",
        timeout_error_text="Timed out waiting for OTP code.",
    )
    if not otp_code:
        return False
    ...
    password = await _ask_for_input(
        ...
        field="2fa_password",
        input_type="password",
        timeout_error_text="Timed out waiting for 2FA password.",
    )
    if not password:
        return False
