...
async def _ask_for_input(
    ...
    try:
        ...
        await send_message(
            relay_url,
            session_id,
            {"type": "error", "text": f"Timed out waiting for {error_label}."},
        )
        return None


async def _notify_auth_success(relay_url: str, session_id: str, result: dict) -> bool:
...
async def _relay_telethon_auth(
    ...
    try:
        ...
        return await _notify_auth_success(relay_url, session_id, result)
    except Exception as e:
        error_msg = str(e)
