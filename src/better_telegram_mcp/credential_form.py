...
def _render_css() -> str:
    """Return the dark-themed CSS for the form."""
    return """
        *, *::before, *::after {
...
def _render_tabs(initial_tab: str) -> str:
...
def _render_form_panels(
    ...
    """Render the Bot/User mode form panels."""
...
def _render_js(submit_url_escaped: str, initial_tab: str) -> str:
    """Return the complex JavaScript logic."""
    return f"""
    <script>
        (function () {{
            ...
            function showStepInput(ns) {{
                ...
                if (container) {{
                    ...
                    // Auto-strip validation styling as soon as user resumes typing
                ...
                promptEl.textContent = ns.prompt || (ns.type === "otp_required" ? "Enter OTP" : "Enter Password");
...
def render_telegram_credential_form(
    ...
    tabs = _render_tabs(initial_tab)
    form_panels = _render_form_panels(
        initial_tab, bot_token_value_attr, phone_value_attr
    )
    js = _render_js(submit_url_escaped, initial_tab)
