...
async def _per_request_sub_scope(
    ...
    if sub:
        ...
        if provider is not None:
            ...
            # Bolt: Use itertools.islice to avoid O(N) list creation of all keys
            keys_preview = itertools.islice(provider.active_clients.keys(), 5)
            keys_short = [k[:12] + "..." if len(k) > 12 else k for k in keys_preview]
            logger.info(
...
def _start_multi_user_http(settings: Settings) -> None:
    ...
    if settings.cf_mode:
        ...
        from ..auth.pending_otp_store import PendingOtpStore
