...
## 2024-11-20 - Disable Auxiliary Controls During Async Submission
**Learning:** When transitioning a form into a loading or processing state (e.g., setting `submitBtn.disabled = true;`), it's easy to forget auxiliary interactive elements like password visibility toggles (`.password-toggle`). Leaving these enabled allows users to interact with them while the primary form is locked, creating inconsistent and confusing UI states.
**Action:** Always comprehensively disable all interactive elements within a form (including custom toggles and secondary buttons) when locking the form for an async submission, and properly re-enable them in both the success and error/catch handlers. Additionally, when reusing or resetting dynamic form containers (like step inputs), explicitly clear the disabled attribute and reset ARIA states (e.g., `aria-pressed="false"`) on these auxiliary controls to maintain sync.
