...
**Learning:** Hiding text (e.g., `color: transparent !important`) to make room for a CSS spinner works well normally, but when an OS-level `prefers-reduced-motion: reduce` query strips animations, the spinner stops. If the pseudo-element displaying the frozen spinner isn't hidden and the text isn't restored, the loading state appears as a blank, frozen button, causing user confusion during async operations.
**Action:** When implementing CSS spinners that replace text, always include overrides within `@media (prefers-reduced-motion: reduce)` to explicitly restore the original text color (`color: inherit` or `#fff`) and hide the spinner (`display: none !important`) so users receive a clear textual loading indicator (e.g., "Connecting...").
